...
public class DebrisGobletHoneyBlock extends FilledGobletBlock {

    public DebrisGobletHoneyBlock(Settings settings) {
        super(settings, DTObjects.EMPTY_DEBRIS_GOBLET, Type.HONEY);
    }
    ...
    protected void getFilledGobletStack(PlayerEntity player, Hand hand, ItemStack toSwitch) {
        if (toSwitch.isOf(DTObjects.EMPTY_DEBRIS_GOBLET_ITEM)) {
            DTUtil.switchStack(player, hand, toSwitch, DTObjects.DEBRIS_GOBLET_OF_HONEY_ITEM);
        }
    }
}
