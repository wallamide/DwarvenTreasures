...
public class MithrilGobletMilkBlock extends FilledGobletBlock {

    public MithrilGobletMilkBlock(Settings settings) {
        super(settings, DTObjects.EMPTY_MITHRIL_GOBLET, Type.MILK);
    }
    ...
    protected void getFilledGobletStack(PlayerEntity player, Hand hand, ItemStack toSwitch) {
        if (toSwitch.isOf(DTObjects.EMPTY_MITHRIL_GOBLET_ITEM)) {
            DTUtil.switchStack(player, hand, toSwitch, DTObjects.MITHRIL_GOBLET_OF_MILK_ITEM);
        }
    }
}
