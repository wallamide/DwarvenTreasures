...
public class SilverGobletMilkBlock extends FilledGobletBlock {

    public SilverGobletMilkBlock(Settings settings) {
        super(settings, DTObjects.EMPTY_SILVER_GOBLET, Type.MILK);
    }
    ...
    protected void getFilledGobletStack(PlayerEntity player, Hand hand, ItemStack toSwitch) {
        if (toSwitch.isOf(DTObjects.EMPTY_SILVER_GOBLET_ITEM)) {
            DTUtil.switchStack(player, hand, toSwitch, DTObjects.SILVER_GOBLET_OF_MILK_ITEM);
        }
    }
}
